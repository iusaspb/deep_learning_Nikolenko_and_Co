...
def show_scatter(sess,data_show,epoch):
    ...
    plt.gray()
    fig.canvas.set_window_title(f'Рис. 10.7. 2D-представление распределения рукописных цифр в датасете MNIST в условном вариационном автокодировщике эпоха {epoch}')
    plt.scatter(z_mu[:, 0], z_mu[:, 1], c=np.argmax(y_sample, 1))
    plt.colorbar()
    plt.show()

def train(sess, batch_size=100, training_epochs=10, display_step=5):
    show_scatter(sess, data_show, 0)
    for epoch in range(training_epochs):
        ...
        for i in range(total_batch):
            ...
            _, c,_reconstr_loss,_latent_loss = sess.run((optimizer, cost, reconstr_loss_mean,latent_loss_mean),
                                                        feed_dict={
                                                            x: xs,
                                                            y: ys,
                                                            eps:np.random.normal(loc=0.0, scale=1.0, size=(batch_size, n_z))})
        ...
        if epoch in show_step:
            show_scatter(sess, data_show, epoch)
    show_scatter(sess, data_show, training_epochs)

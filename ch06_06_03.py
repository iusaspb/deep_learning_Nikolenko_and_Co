#
# LSTM c одним слоем, как в  ch06_06_01
# логирование и семплирование происходит не по эпохам. а по батчам, в отличии от ch06_06_01
#
import os
import csv
import numpy as np
...
checkpoint_fname = 'checkpoints/ch06_06_03.hdf5'
log_fname = 'logs/ch06_06_03.log'
output_fname='data/ch06_06_03_samplers.txt'
batch_size = 16
output_dim=128
num_epochs=4#1000
#  составить список всех символов в тексте и построить индексы
with open(input_fname, 'r', encoding='utf_8') as log_rec:
    for line in log_rec:
...
if os.path.isfile(checkpoint_fname):
    ...
    model = Sequential()
    model.add(LSTM(output_dim=output_dim, activation='tanh',return_sequences=True, input_dim=num_chars))
...
num_params = size_LSTM(num_chars,output_dim)
num_params+=output_dim*num_chars +  num_chars  # Dense
...
with open(log_fname,'r') as log_file:
    ...
    fig.canvas.set_window_title('Рис. 6.14. Графики функции потерь и точности на тренировочном и валидационном множествах модель с одним слоем LSTM, «Евгений Онегин»')
